...
function assignRoles() {
  ...
  const loadBlockchainData = async () => {
    ...
    if (networkData) {
      ...
      var i;
      const rmsCtr = await supplychain.methods.rawMaterialsCounter().call();
      const rms = {};
      for (i = 0; i < rmsCtr; i++) {
        rms[i] = await supplychain.methods.RMS(i + 1).call();
      }
      setRMS(rms);
      const manCtr = await supplychain.methods.manufacturerCounter().call();
      const man = {};
      for (i = 0; i < manCtr; i++) {
        man[i] = await supplychain.methods.MAN(i + 1).call();
      }
      setMAN(man);
      const disCtr = await supplychain.methods.distributorCounter().call();
      const dis = {};
      for (i = 0; i < disCtr; i++) {
        dis[i] = await supplychain.methods.DIS(i + 1).call();
      }
      setDIS(dis);
      const retCtr = await supplychain.methods.retailerCounter().call();
      const ret = {};
      for (i = 0; i < retCtr; i++) {
  ...
  const handlerSubmitRMS = async (event) => {
    ...
    try {
      var receipt = await SupplyChain.methods
        .addRawMaterialSupplier(RMSaddress, RMSname, RMSplace)
